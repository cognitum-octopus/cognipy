...
public class MultiplyNumbers extends org.apache.jena.reasoner.rulesys.builtins.BaseBuiltin
{
	@Override
	...
	public boolean bodyCall(org.apache.jena.graph.Node[] args, int length, RuleContext context)
	{
		...
		if (allInts)
		{
			...
			if (n0.isLiteral())
			{
				return (Long)RuleExtensions.getValFromJenaLiteral(n0.getLiteralValue()) == product;
			}
			else
			{
		...
		else
		{
			...
			if (n0.isLiteral())
			{
				return (Double)RuleExtensions.getValFromJenaLiteral(n0.getLiteralValue()) == product;
			}
			else
			{
