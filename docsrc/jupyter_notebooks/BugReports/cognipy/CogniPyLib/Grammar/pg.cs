...
public class ParserGenerate : SymbolsGen
{
    ...
    public override void SetName()
    {
        m_tok = m_lexer.Next();
        m_namespace = true;
        m_outname = m_tok.yytext;
    }

    public override void SetPartial()
    {
        m_partial = true;
    }
	...
	{ // may update s to a new node type
		...
		{ // get a genuinely new identifier
            //newname = partial?
            //    String.Format("{0}_{1}_{2}", bas.yytext, m_outname, variant):
            newname=    String.Format("{0}_{1}", bas.yytext, Guid.NewGuid().ToString("N"));
	...
	internal void ParseProduction() 
	{
        ...
        { // lhs not defined in %symbol statement and not previously a lhs
            ...
            m_outFile.WriteLine("public partial class " + lhs.yytext + " : " + (m_partial ? "PartialSymbol" : "SYMBOL") + " {");
            if (m_partial)
            {
                m_outFile.WriteLine("public override string yyname_" + m_outname + " { get { return \"" + lhs.yytext + "\"; }}");
                m_outFile.WriteLine("public override int yynum_" + m_outname + " { get { return " + lhs.m_yynum + "; }}}");
            }
            else
            {
